...
const colors = { 연차: "#e06c75", 근무: "#26775f", GY: "#7b61c9", 기타: "#3b82b8" };

export function AttendanceCalendar() {
  const members = useLocalCollection<Member>("tsv-members");
  const schedules = useLocalCollection<Schedule>("tsv-schedules");
  const [holidays, setHolidays] = useState<Holiday[]>([]); const [open, setOpen] = useState(false); const [memberOpen, setMemberOpen] = useState(false); const [selectedDate, setSelectedDate] = useState("");

  async function loadHolidays(year: number) { try { const response = await fetch(`/api/holidays/${year}`); setHolidays(await response.json()); } catch { setHolidays([]); } }
  ...
  function submitSchedule(event: FormEvent<HTMLFormElement>) { event.preventDefault(); const data = new FormData(event.currentTarget); const memberId = String(data.get("memberId")); const member = members.items.find((item) => item.id === memberId); if (!member) return; schedules.add({ memberId, memberName: member.name, type: String(data.get("type")) as Schedule["type"], start: String(data.get("start")), end: String(data.get("end")), memo: String(data.get("memo")) }); setOpen(false); }
  function submitMember(event: FormEvent<HTMLFormElement>) { event.preventDefault(); const data = new FormData(event.currentTarget); members.add({ name: String(data.get("name")), role: "", email: "", annualLeave: Number(data.get("annualLeave")) }); setMemberOpen(false); }
  ...
  return <FeatureLayout icon="🗓️" title="근태캘린더" description="연차, 근무, GY 일정을 달력에서 함께 관리합니다." action={<div className="action-row"><button className="secondary-button" onClick={() => setMemberOpen(true)}>＋ 멤버 추가</button><button className="primary-button" onClick={() => { setSelectedDate(new Date().toISOString().slice(0, 10)); setOpen(true); }}>＋ 일정 추가</button></div>}>
    <div className="leave-summary">{leaveCounts.length === 0 ? <span>멤버를 추가하면 연차 현황이 표시됩니다.</span> : leaveCounts.map((member) => <div className="leave-stat" key={member.id}><b>{member.name}</b><span>사용 {member.used}일</span><strong>잔여 {Math.max(0, member.annualLeave - member.used)}일</strong></div>)}</div>
    <section className="calendar-panel"><FullCalendar plugins={[dayGridPlugin, interactionPlugin, listPlugin]} initialView="dayGridMonth" locale={koLocale} headerToolbar={{ left: "prev,next today", center: "title", right: "dayGridMonth,listMonth" }} buttonText={{ today: "오늘", month: "월", list: "목록" }} events={events} dateClick={openDate} editable selectable eventClick={(info) => { if (!info.event.id.startsWith("holiday-")) { if (window.confirm(`${info.event.title} 일정을 삭제할까요?`)) schedules.remove(info.event.id); } }} datesSet={(info) => loadHolidays(info.view.currentStart.getFullYear())} height="auto" />
      <p className="calendar-help">날짜를 누르면 일정을 추가할 수 있고, 등록된 일정을 누르면 삭제할 수 있습니다. 빨간색은 대한민국 공휴일입니다.</p></section>
